...
from rl_book.methods.method import train
...
@pytest.mark.parametrize("method_name", [OnPolicyMC, OffPolicyMC, OffPolicyMCNonInc])
def test_mc(env_train: ParametrizedEnv, method_name: Callable):
    observation_space, _ = get_observation_action_space(env_train)
    method = method_name(env_train)
