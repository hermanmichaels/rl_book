...
class MCMethod(Algorithm):
    def __init__(self, env):
        ...
        # TODO: misused
        self.pi = (
            np.ones((self.env.env.observation_space.n, self.env.env.action_space.n)).astype(np.int32) / self.env.env.action_space.n
        )
    ...
    def finalize(self, episode, step):
        ...
        for t in range(len(episode) - 2, -1, -1):
            ...
            if (s, a) not in prev_s:
                ...
                if not all(self.Q[s, a] == self.Q[s, 0] for a in range(self.env.env.action_space.n)):
                    A_star = np.argmax([self.Q[s, a] for a in range(self.env.env.action_space.n)])
                    for a in range(self.env.env.action_space.n):
                        self.pi[s, a] = (
                            1 - self.env.eps(step) + self.env.eps(step) / self.env.env.action_space.n
                            if a == A_star
...
class OffPolicyMC(MCMethod):
    ...
    def get_greedy_policy(self):
        return np.argmax(np.asarray([[self.Q[s, a] for a in range(self.env.env.action_space.n)]  for s in range(self.env.env.observation_space.n)]), 1)
