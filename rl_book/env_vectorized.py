...
class VectorizedReplayBuffer:
    ...
    def get_batch(self, device):
        ...
        actions = torch.from_numpy(self.act_buf[:self.t]).flatten(0, 1).to(self.device).long()
        rewards = torch.from_numpy(self.rew_buf[:self.t]).flatten(0, 1).to(self.device) 
        next_states = torch.from_numpy(self.obs_next_buf[:self.t]).flatten(0, 1).to(self.device) 
...
class VectorizedEnv(Generic[E]): # TODO: Generic, ABC
    ...
    def agent_selection(self):
        agent_selections = [env.env.agent_selection for idx, env in enumerate(self.envs) if self.status[idx] != AgentStatus.TERMINATED]
        ...
        assert all(s == first_agent_selection for s in agent_selections), (
    ...
    def step(self, action):
        ...
        for idx in alive_idxs:
            self.envs[idx].env.step(action[idx] if self.status[idx] == AgentStatus.ALIVE else None)
    ...
    def last(self):
        ...
        for idx in alive_idxs:
            (
                ...
                truncation,
                _,
            ) = self.envs[idx].env.last()  # type: ignore

            self.obs_np[idx]  = self.envs[0].obs_to_state(observation["observation"], obs_mode=ObsMode.RASTERIZED).detach().cpu().numpy()
            self.mask_np[idx] = observation["action_mask"].astype(bool, copy=False) # why?
        ...
        return self.obs_np.copy(), self.mask_np.copy(), self.rew_np.copy(), self.done_np.copy()
    
    def rewards(self, idx, is_player):
        player = self.envs[0].players[self.player_pos if is_player else 1 - self.player_pos]
