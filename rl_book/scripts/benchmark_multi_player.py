...
from rl_book.methods.td_approx import SemiGradientSarsaCNN, SemiGradientSarsaLinear, SemiGradientSarsaNCNN
...
def get_env(env_name: str, render_mode=None):
    ...
    if env_name == "TicTacToe":
        env = TicTacToeEnv(tictactoe_v3.env(render_mode=render_mode), 0.95, obs_mode=ObsMode.RASTERIZED)
...
def benchmark_multi_player(env_name: str, load_weights: bool) -> None:
    ...
    methods = [
        ...
        MethodWithStats(SemiGradientSarsaCNN(env, load_weights=load_weights)),
        MethodWithStats(SemiGradientSarsaNCNN(env, load_weights=load_weights))
    ]
